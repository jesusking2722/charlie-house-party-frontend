...
const initialSteps: StepperItem[] = [
    ...
    {icon: 'solar:hand-shake-bold-duotone', label: 'Open for proposals', completed: false},
    {icon: 'solar:shield-bold-duotone', label: 'Terms confirmation', completed: false},
    {icon: 'solar:wineglass-bold-duotone', label: 'In Party', completed: false},
    {icon: 'solar:hand-stars-bold-duotone', label: 'Review exchange', completed: false},
];
...
const PartyPreview = () => {
    ...
    const handleStickersCheck = (sticker: StickerCheckbox) => {
        setStickers(stickers.map(s => s._id === sticker._id ? {...s, isChecked: !s.isChecked} : s));
    };
    ...
    return (
        <div className="w-[80%] mx-auto py-8">
            <div className='w-full flex flex-row items-center justify-between mb-8'>
                ...
                <div className='flex flex-row items-center gap-2'>
                    <Tooltip message='Share party'>
                        <IconButton icon='solar:share-line-duotone' onClick={() => setShareOpen(true)}/>
                    </Tooltip>
                    <Button type='primary' label='Apply' icon='solar:document-add-bold-duotone'
                            onClick={() => setIsApplying(true)}/>
                </div>
            </div>
            <div className="w-full flex flex-1 flex-row items-start justify-between gap-14">
                <div
                    className="flex-1 flex flex-col gap-4 bg-white/10 backdrop-blur-sm border border-white hover:border-[#c4f70f] shadow-lg rounded-xl p-6 transition-all duration-300 ease-in-out">
                ...
                <div
                    className="py-4 px-8 rounded-xl bg-white/10 backdrop-blur-sm shadow-lg border border-white flex flex-col">
                    <div className="flex flex-row items-center gap-8">
                        ...
                        <div className="flex-1 flex flex-col gap-2">
                            ...
                            <div className="flex flex-row items-center gap-2">
                                ...
                                <Icon icon={`flag:${selectedParty?.creator?.country?.toLowerCase()}-4x3`}
            ...
            {isApplying && (
                ...
                >
                    <div className='flex flex-row items-center gap-2'>
                        ...
                        <Icon icon='solar:document-add-bold-duotone' className='text-green-500 w-8 h-8'/>
                    </div>
                    <div className='w-full flex flex-col gap-4'>
                        <Alert type='success' title='Successful tip' message='Stickers is the key to winning bid'/>
                        <p className='text-sm text-black font-semibold'>Your <strong
                            className='text-green-500'>stickers</strong>: </p>
                        <div
                            className='w-full h-[250px] overflow-auto bg-white/10 backdrop-blur-sm border border-[#c4f70f] rounded-xl p-6'>
                            <StickerCheckboxGroup stickers={stickers} onCheck={handleStickersCheck}/>
                        </div>
                        {stickers.filter(s => s.isChecked).length > 0 && (
                            <div className='w-full flex'>
                                <p className='text-sm text-black'>Total selected: <strong
                                    className='text-green-500 text-sm'>{stickers.filter(s => s.isChecked).length}</strong>
                                </p>
                            </div>
                        )}
                    </div>
                    <div className='w-full flex flex-col gap-6'>
                        <Textarea placeholder='Type here...' invalidTxt='Must be between 150 and 1000 letters'
                                  value={apply}
                                  onChange={setApply}/>
                        <Button type='primary' label='Apply' icon='solar:document-add-bold-duotone'/>
                    </div>
            ...
            {user?.membership === 'premium'
                ...
                : (<Alert type='info' title='Tip for winner'
