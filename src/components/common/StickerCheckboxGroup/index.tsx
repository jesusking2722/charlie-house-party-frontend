...
const StickerCheckboxGroup: FC<StickerCheckboxGroupProps> = ({stickers, onCheck}) => {
    return (
            ...
            {stickers.map((sticker, index) => (
                <button
                    ...
                    className={`hover:shadow-lg p-2 w-[120px] h-[120px] border flex flex-col items-center justify-center rounded-lg cursor-pointer transition-all duration-300 ease-in-out ${
                        sticker.isChecked ? "border-[#c4f70f] shadow-xl bg-[#c4f70f]" : "border-gray-300 bg-white/20 backdrop-blur-sm"
                    }`}
                    onClick={() => {
                        onCheck(sticker);
                    }}
                >
                    ...
                    />
                    <span
                        className={`text-xs font-semibold ${sticker.isChecked ? 'text-black' : 'text-gray-400'}`}>{sticker.name}</span>
                </button>
            ))}
        </div>
    );
